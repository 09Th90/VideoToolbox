...
# -*- coding: utf-8 -*-
"""视频工具箱 GUI v1.9.3
图形界面：视频下载 + 视频库 + 音视频合并 + 本地字幕生成（Whisper）+ 字幕校准 + B站自动投稿
复用 video_toolbox.py 的全部核心逻辑（同目录 / 已随 exe 打包）
v1.9.3：新增「字幕校准」标签页——以子进程方式调用工作区统一校准脚本
        subtitle_calib_merged.py（只调用、不修改原脚本）：术语级原位校准
        （序号/时间轴/换行/BOM 一字不动），支持 双语/韩/日/终末地/明日方舟/
        中文行专属/战双帕弥什/综合OST 八种片源模式，可选长句拆行、英文行修正、
        改动对照报告与仅统计命中。
v1.9.2：① 投稿信息填写改回脚本（DOM）为主通道——分区/简介/话题均脚本优先，
        AI 屏幕识别降级为兜底（AI 未启用也能完成填写）；② 投稿页新增「上传视频」
        按键——仅上传不填表不投稿，用于提前上传与验证上传通道；③ 全功能报错检测：
        上传失败/卡死立即报错（不再空等 30 分钟超时）、各检测/投稿阶段失败自动
        截图留证、界面回调未捕获异常统一写入 logs\\gui_errors.log 并状态栏提示。
...
class CalibTab(ttk.Frame):
    """字幕校准：以子进程方式调用工作区统一校准脚本 subtitle_calib_merged.py
    （只调用、不修改原脚本）。术语级原位校准——序号/时间轴/空行/换行/BOM
    一字不动；不写输出文件时仅做术语命中统计。"""
    ...
    def __init__(self, parent, app):
        ...
        ttk.Label(self, text="字幕校准", style="Title.TLabel").pack(anchor="w")
        if os.path.isfile(self._script):
            ttk.Label(self, text="调用统一校准脚本 subtitle_calib_merged.py 做名词级术语校准；"
                                 "勾选「仅统计」时不改写文件，只报告命中数",
                      style="Muted.TLabel").pack(anchor="w", pady=(2, GAP + 2))
        else:
            ttk.Label(self, text=f"校准脚本缺失：{self._script}",
                      style="Muted.TLabel", foreground=DANGER).pack(
                anchor="w", pady=(2, GAP + 2))

        ttk.Label(self, text="字幕文件（.srt）", style="Head.TLabel").pack(anchor="w")
        row = ttk.Frame(self)
        row.pack(fill="x", pady=(4, GAP))
        self.in_var = tk.StringVar()
        in_entry = ttk.Entry(row, textvariable=self.in_var)
        in_entry.pack(side="left", fill="x", expand=True)
        self.in_undo = attach_entry_undo(in_entry, self.in_var)
        ttk.Button(row, text="浏览…", command=self.browse_in).pack(side="left",
                                                                   padx=(8, 0))

        mrow = ttk.Frame(self)
        mrow.pack(fill="x", pady=(0, GAP))
        ttk.Label(mrow, text="片源模式:", style="Head.TLabel").pack(side="left")
        self.mode_box = ttk.Combobox(mrow, state="readonly", width=34,
        ...
        ttk.Checkbutton(opt, text="长句拆两行（--layout，≥14 字在标点处拆，不切官方专名）",
                        variable=self.layout_var).pack(anchor="w")
        self.fixen_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt, text="修正英文/参考行（--fix-en，仅双语模式生效）",
                        variable=self.fixen_var).pack(anchor="w")
        self.report_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(opt, text="生成改动对照报告（同名 .md）",
                        variable=self.report_var).pack(anchor="w")
        self.stats_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(opt, text="仅统计术语命中，不写输出文件",
                        variable=self.stats_var).pack(anchor="w")

        ttk.Label(self, text="输出文件（留空自动：同目录 原名.calib.srt，重名自动加序号）",
                  style="Head.TLabel").pack(anchor="w")
        orow = ttk.Frame(self)
        orow.pack(fill="x", pady=(4, GAP))
        self.out_var = tk.StringVar()
        out_entry = ttk.Entry(orow, textvariable=self.out_var)
        out_entry.pack(side="left", fill="x", expand=True)
        self.out_undo = attach_entry_undo(out_entry, self.out_var)
        ttk.Button(orow, text="浏览…", command=self.browse_out).pack(side="left",
                                                                     padx=(8, 0))

        row3 = ttk.Frame(self)
        row3.pack(fill="x", pady=(0, 4))
        self.stage_var = tk.StringVar(value="")
        ttk.Label(row3, textvariable=self.stage_var, style="Muted.TLabel").pack(
            side="left", padx=(2, 8))
        self.run_btn = ttk.Button(row3, text="开始校准", style="Accent.TButton",
                                  command=self.start)
        self.run_btn.pack(side="right")

        self.log = LogBox(self, height=8)
        self.log.widget.pack(fill="both", expand=True)

    def browse_in(self):
        p = filedialog.askopenfilename(
            initialdir=self.in_var.get() or engine.SCRIPT_DIR,
    ...
    def start(self):
        if self.running:
            messagebox.showinfo("提示", "校准任务进行中，请稍候")
            return
        if not os.path.isfile(self._script):
            messagebox.showerror("错误", f"校准脚本缺失：{self._script}")
            return
        src = engine.clean_path(self.in_var.get())
        if not src or not os.path.isfile(src):
            messagebox.showwarning("提示", "请先选择一个 .srt 字幕文件")
            return
        mode_flag = self.MODES[self.mode_box.current()][1]
        stats_only = self.stats_var.get()

        out = ""
        if not stats_only:
            out = engine.clean_path(self.out_var.get())
            if not out:
        ...
        if self.layout_var.get():
            args.append("--layout")
        if self.fixen_var.get():
            args.append("--fix-en")

        self.running = True
        self.run_btn.configure(state="disabled")
        self.stage_var.set("正在校准…")
        self.log.line(f"[校准] 模式: {self.mode_box.get()}"
                      + ("（仅统计）" if stats_only else ""), "dim")
        self.log.line(f"[校准] 输入: {os.path.basename(src)}", "dim")
        if out:
            self.log.line(f"[校准] 输出: {out}", "dim")
        threading.Thread(target=self._worker, args=(args, out),
                         daemon=True).start()

    def _worker(self, args, out):
        """子进程运行校准脚本，逐行回传输出；异常与非零退出均报错。"""
        try:
            env = dict(os.environ, PYTHONIOENCODING="utf-8")
            proc = engine.popen_process(
                args, cwd=os.path.dirname(self._script),
    ...
    def on_cal_done(self, rc, out):
        ...
        if rc == 0:
            self.stage_var.set("校准完成")
            self.app.status_var.set("字幕校准完成")
            self.log.line("[OK] 校准完成" +
                          (f"，输出: {out}" if out else "（仅统计模式，未改写文件）"),
                          "ok")
        else:
            self.stage_var.set("校准失败")
...
class UploadTab(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        self.upload_only_btn = ttk.Button(
            row2, text="上传视频", command=self.start_upload_only,
            state="normal" if ready else "disabled")
        self.upload_only_btn.pack(side="right", padx=(0, 8))
    ...
    def _set_detect_buttons(self, state: str) -> None:
        """统一设置所有检测/上传按钮的可用状态（disabled / normal）。

        检测按钮（分区/合集/话题）与「上传视频」互斥：任一后台任务运行时全部禁用，
        避免并行启动多个浏览器任务导致资源竞争与界面卡死。
        """
        for btn in (self.area_btn, self.collection_btn, self.topic_btn,
                    self.upload_only_btn):
            try:
    ...
    def start_upload_only(self):
        """仅上传所选视频到 B 站投稿页并等待就绪：用于提前上传/验证上传通道，
        附完整报错检测（失败立即提示并保存截图）。"""
        if self.running:
            messagebox.showinfo("提示", "已有投稿/上传任务在进行中，请等待其结束")
            return
        ...
        self.stage_var.set("正在上传视频（只上传不投稿，请勿关闭弹出的浏览器）…")
        self.log.line(f"[上传] 仅上传模式 | 视频: {os.path.basename(video)}", "dim")
        threading.Thread(target=self._run_upload_only, args=(video,),
                         daemon=True).start()

    def _run_upload_only(self, video):
        """主进程内常驻 asyncio 循环：与检测/投稿共享同一个浏览器会话。"""
        self.app.loop.run_in_executor(
            None, self._upload_only_in_loop, video)

    def _upload_only_in_loop(self, video):
        """在后台 asyncio loop 中执行仅上传；统一日志经 self.app.q 回主线程。"""
        import asyncio
        import importlib
        ...
        try:
            ...
            self.app.q.put(("up_log", f"[上传] 引擎加载失败: {e}", True))
            self.app.q.put(("upload_only_done", 1))
            return
        ...
        handler = None
        try:
            up_config.apply_cli_overrides(video_path=video)
            ...
            try:
                future = asyncio.run_coroutine_threadsafe(
                    runner.run_upload_only(on_stage=on_stage,
            ...
            self.app.q.put(("up_log", f"[上传] 引擎调用失败: {e}", True))
            rc = 1
        self.app.q.put(("upload_only_done", rc))

    def on_upload_only_done(self, rc):
        """主线程处理仅上传结果。"""
        self.running = False
        ready = engine.uploader_env_ready()
        self._set_detect_buttons("normal" if ready else "disabled")
        self.up_btn.configure(state="normal" if ready else "disabled")
        if rc == 0:
            self.stage_var.set("视频已上传就绪（未投稿）")
            self.app.status_var.set("视频上传完成")
            self.log.line("[OK] 视频上传完成，投稿页已就绪；"
                          "可继续点「开始投稿」或在浏览器中人工检查", "ok")
        elif rc == 2:
            self.stage_var.set("失败：视频不存在")
            self.log.line("[错误] 视频文件不存在，请检查路径", "err")
        else:
            self.stage_var.set("上传失败")
            self.log.line(f"[错误] 上传失败（退出码 {rc}）。"
                          f"详情与失败截图见 {engine.UPLOADER_LOGS_DIR}\\screenshots",
                          "err")
    ...
    def _ai_detect_title_lang(self, title):
        ...
        def work():
            try:
                ...
                self.app.q.put(("up_log", f"[AI] 标题语言识别异常: {e}", True))
                code = ""
...
class App(tk.Tk):
    def __init__(self, argv):
        ...
        # 全局报错检测（v1.9.2）：任何界面回调/功能入口抛出未捕获异常时，
        # 记录堆栈到 logs\gui_errors.log 并在状态栏红色提示，不再静默吞掉
        ...
        ttk.Label(self, text="并行下载 ・ 独立打包 ・ 自动视频库 ・ 音视频合并 ・ 本地字幕生成 ・ 字幕校准 ・ B站自动投稿",
        ...
        add_page(CalibTab, "  字幕校准  ", "cal_tab")
    ...
    def _on_callback_error(self, exc, val, tb):
        """Tkinter 回调未捕获异常的全局处理：写日志文件 + 状态栏提示。"""
        import traceback
        ...
        try:
            ...
            pass
        try:
            self.status_var.set(f"界面功能出错：{val}（详见 logs\\gui_errors.log）")
        except Exception:  # noqa: BLE001
            pass
        traceback.print_exc()
        ...
        try:
            while True:
                ...
                if kind == "detect_done":
                    ...
                    self.cal_tab.log.line(msg[1], "dim")
                elif kind == "cal_done":
                    self.cal_tab.on_cal_done(msg[1], msg[2])
                elif kind == "up_log":
                    ...
                    self.up_tab.on_upload_only_done(msg[1])
...
def main():
    ...
    elif shot_tab in ("calib", "download", "library", "merge", "subtitle"):
        attr = {"calib": "cal_tab", "download": "dl_tab", "library": "lib_tab",
                "merge": "mg_tab", "subtitle": "sub_tab"}[shot_tab]
        app.after(900, lambda: app.nb.select(app.page_host[attr]))
    if shot_tab == "calib_run":
        # 端到端演示：自动填入测试字幕并点击「开始校准」（验证子进程链路）
        def _calib_run():
            app.nb.select(app.page_host["cal_tab"])
            src = os.path.join(engine.APP_DIR, "build", "test_calib_input.srt")
            app.cal_tab.in_var.set(src)
            app.cal_tab.out_var.set("")
            app.cal_tab.start()
        app.after(900, _calib_run)
