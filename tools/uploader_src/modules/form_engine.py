# -*- coding: utf-8 -*-
"""
模块三：智能表单填写引擎（开发书 3.3 / R3；v1.9.2 脚本为主通道）。

设计原则（v1.9.2，对应用户最新要求：改回使用脚本完成投稿信息填写）：
1. 全部字段（标题/简介/分区/声明/合集等）优先走「DOM 脚本通道」：
   page.evaluate 定位 → CDP 可信点击 → 键盘逐字输入 → 读回校验；
2. AI 屏幕识别（截屏 → 视觉定位 → 拟人鼠标点击/滚轮滚动）保留为兜底通道：
   仅当 DOM 通道失败且 AI 代理可用时自动回退，AI 未启用不影响脚本通道；
3. 每次填写前重新定位，不缓存元素引用；单字段失败隔离并截图留证。
"""
...
# ---------------------------------------------------------------------------
# 分区选择：AI 屏幕识别兜底通道（v1.9.2 起为 DOM 脚本通道失败时的回退）
# ---------------------------------------------------------------------------
...
async def _select_dropdown_steps(page: Page, field_key: str, field: dict,
                                 steps, agent=None) -> dict:
    """DOM 脚本通道：打开下拉 → 逐级按文本点击选项（含 AI 视觉单级兜底）。"""
    ...
    return {"ok": True, "selected": list(steps), "strategy": "dom"}
...
async def fill_select_field(page: Page, field_key: str, field: dict,
    ...
    与人工在B站原始页面上的操作保持一致。
    v1.9.2（用户要求改回脚本填写）：全部下拉走 DOM 脚本通道为主；
    分区（zone）DOM 通道失败且有 AI 代理时，回退屏幕识别（拟人点击+滚轮）。
    """
    value = field["value"]
    ...
    try:
        return await _select_dropdown_steps(page, field_key, field, steps, agent)
    except Exception as e:  # noqa: BLE001
        if field_key == "zone" and agent is not None:
            logger.warning("分区脚本通道失败，回退 AI 屏幕识别: %s", e)
            return await _fill_zone_by_vision(page, agent, steps)
        raise
...
async def fill_topics_field(page: Page, field_key: str, field: dict,
    ...
    v1.9.2（用户要求脚本填写）：DOM 文本扫描为主通道；脚本未命中的话题
    在有 AI 代理时回退屏幕识别通道逐个补点。
    """
    ...
    # 通道 2：脚本未命中的话题回退 AI 屏幕识别
    if missed and agent is not None:
        logger.info("脚本通道未命中话题 %s，回退 AI 屏幕识别补点", missed)
        res = await _fill_topics_by_vision(page, agent, missed)
        for t in res.get("clicked", []):
            if t in missed:
                missed.remove(t)
                clicked.append(t)
    for t in missed:
        logger.warning("话题「%s」未在“参与话题”区找到，跳过"
                       "（可在浏览器中手动参与）", t)
