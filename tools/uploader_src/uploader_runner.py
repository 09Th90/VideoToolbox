...
# ---------------------------------------------------------------------------
# 仅上传模式（GUI「上传视频」按钮）：只完成 登录确认 → 上传视频 → 就绪确认，
# 不填表不投稿；用于提前上传/验证上传通道，附完整报错检测与失败截图
# ---------------------------------------------------------------------------
async def run_upload_only(*, session: BrowserSession | None = None,
                          page: Page | None = None,
                          use_browser_manager: bool = False,
                          on_stage=None) -> int:
    """只上传视频并等待就绪，不填表不投稿。返回 0 成功 / 1 失败 / 2 视频不存在。"""
    def stage(key: str) -> None:
        if on_stage is not None:
            try:
                on_stage(key, STAGE_LABELS.get(key, key))
    ...
    if not video or not Path(video).exists():
        logger.error("视频文件不存在: %s（请先选择视频文件）", video or "(未指定)")
        return 2

    own_session = session is None
    if own_session and not use_browser_manager:
        session = BrowserSession()
    elif page is None and not use_browser_manager:
        raise ValueError("复用浏览器会话时必须同时提供 page")
    ...
    try:
        ...
        stage_name = "upload"
        stage("upload")
        await upload_video(page, video)

        stage("done")
        logger.info("仅上传模式完成：视频已上传并就绪（未填表、未投稿）")
        return 0
    except Exception as e:  # noqa: BLE001
        logger.exception("仅上传模式在阶段 [%s] 失败: %s", stage_name, e)
        await save_failure_screenshot(page, "upload_only_" + stage_name)
        return 1
    finally:
        if own_session and not use_browser_manager:
            await session.stop()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--upload-only", action="store_true",
                        help="仅上传视频并等待就绪，不填表不投稿（需配合 --video）")
...
def main() -> None:
    ...
    if args.upload_only:
        if not args.video:
            logger.error("--upload-only 需配合 --video 指定视频文件")
            sys.exit(2)
        apply_args(args)
        logger.info("仅上传模式，日志: %s", log_path)
        rc = asyncio.run(run_upload_only())
        sys.exit(rc)
    if not args.task:
