...
class SubtitleThread(QThread):
    ...
    def __init__(self, task: SubtitleTask):
        ...
        self.optimizer = None
        # v1.11.0：保存翻译器引用，取消（stop）时才能关掉它的线程池——
        # 之前它是 run() 里的局部变量，UI 的停止够不到，翻译线程池会继续
        # 把剩余批次跑完（表现就是取消后 cmd 里仍在刷 Rate Limit 重试）
    ...
    def stop(self):
        ...
        try:
            ...
            # v1.11.0：再停止翻译器（之前是 run() 局部变量，取消后线程池
            # 继续把剩余批次跑完——cmd 刷 Rate Limit 重试的根源）
            if getattr(self, "translator", None):
                try:
                    self.translator.stop()  # type: ignore
                except Exception as e:
...
class RetranslateThread(QThread):
    ...
    def __init__(self, selected_data: dict, subtitle_config: SubtitleConfig, file_name: str = ""):
        ...
        self.done = 0
        self.translator = None  # v1.11.0：取消时关线程池用

    def stop(self):
        """停止重译：关掉翻译器线程池（v1.11.0 补齐，UI 的停止按钮可调用）"""
        t = getattr(self, "translator", None)
        if t:
            try:
                t.stop()
            except Exception as e:  # noqa: BLE001
                logger.error(f"停止重译线程时出错：{str(e)}")
