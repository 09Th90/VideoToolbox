...
(args) => {
    ...
    while ((node = walker.nextNode())) {
        ...
        if (t && t.length <= 12 && busyKeywords.some(k => t.includes(k))) {
            const p = node.parentElement;
            if (p && isVisible(p)) { busy = true; break; }
        }
    }
    return { readyHit, busy };
}
"""
...
async def wait_until_upload_ready(page: Page, timeout: float | None = None) -> None:
    ...
    logger.info("等待视频上传与初始化完成（超时 %.0f 秒）...", timeout)
    ...
    raise PWTimeoutError(
        f"等待上传完成超时（{timeout:.0f}s）。请检查网络/带宽，或在 config.TIMEOUT 中调大 "
